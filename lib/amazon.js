...
async function callMwsMethod(method, version, params) {
  const url = method.toLowerCase().includes('report') ? this.config.environment.Reports.mwsEndpoint : this.config.environment.mwsEndpoint;
  const required = {
  ...
  try {
    ...
    if (response.headers && response.headers['content-type'] === 'text/xml') {
      ...
      if (result.ErrorResponse) {
        ...
        throw error.apiError(apiError.Code, apiError.Message, result);
      }
  ...
  try {
    ...
    for (let i = 0; i < required.length; i += 1) {
      if (!response[required[i]]) {
        throw error.missingParameter(`Missing parameter on SNS response: ${required[i]}`);
      }
    }

    if (response.SignatureVersion !== 1 && response.SignatureVersion !== '1') {
      throw error.invalidSignatureVersion(`Unknown SNS Signature version: ${response.SignatureVersion}`);
    }
    ...
    if (parsed.protocol !== 'https:' || parsed.path.substr(-4) !== '.pem' || !defaultHostPattern.test(parsed.host)) {
      throw error.invalidCertificateDomain('The certificate is located on an invalid domain.');
    }
    ...
    if (!isValid) throw error.signatureMismatch('Signature mismatch, unverified response');
