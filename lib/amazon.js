...
function callApiMethod(action, accessToken) {
...
function callMwsMethod(method, version, params) {
  return new Promise((resolve, reject) => {
    const self = this;
    ...
    request(opts, (err, res, body) => {
      ...
      resolve(parseMwsResponse(method, res.headers, body));
    });
  });
}
 ...
 */
function parseMwsResponse(method, headers, response) {
  return new Promise((resolve, reject) => {
    ...
    if (headers && headers['content-type'] === 'text/xml') {
      xml2js.parseString(response, { explicitArray: false }, (err, result) => {
        ...
        if (result.ErrorResponse) {
          ...
          reject(error.apiError(err.Code, err.Message, result));
        }
        resolve(new Response(method, result));
      });
    } else {
      resolve(new Response(method, { Response: response }));
    }
  });
}
